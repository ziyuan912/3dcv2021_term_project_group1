...
base_folder = './testcases/1'
img_folder = os.path.join(base_folder, 'images')
json_file = os.path.join(base_folder, 'sfm_data.json')
fg_folder = os.path.join(base_folder, 'fg')
bg_base_folder = './nex/runs/video_output'
bg_folders = ['gray', 'black', 'red']
...
def main():
    ...
    for idx, (img_file, R, T) in enumerate(tqdm(zip(img_files, Rs, Ts), total=len(Rs))):
        ...
        if idx <= press_idx[1]:
            bg_folder = os.path.join(bg_base_folder, bg_folders[0])
        elif idx <= press_idx[2]:
            bg_folder = os.path.join(bg_base_folder, bg_folders[1])
        else:
            bg_folder = os.path.join(bg_base_folder, bg_folders[2])

        img2 = cv2.imread(os.path.join(bg_folder, 'frame_'+"%04d"%idx+'.png'))
        ...
        for i, v in enumerate(grid):
            if v == True:
                if idx <= press_idx[0]:
                    img[i//img.shape[1], i%img.shape[1], :] = [100, 100, 100]
                else:
                    img[i//img.shape[1], i%img.shape[1], :] = img2[i//img.shape[1], i%img.shape[1], :] 
        img = cv2.resize(img, (img.shape[1]//4, img.shape[0]//4))
        
        img3 = cv2.imread(os.path.join(fg_folder, "%05d"%idx+'.png'), cv2.IMREAD_UNCHANGED)
        img[img3[:, :, 3] != 0] = img3[img3[:, :, 3] != 0][:, :3]
